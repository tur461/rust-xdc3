...
impl<T, F> CallFuture<T, F> {    /// Create a new CallFuture wrapping the inner future.
    ...
    F: Future<Item = rpc::Value, Error = Error>,
{
    ...
    fn poll(&mut self) -> Poll<T, Error> {
        match self.inner.poll() {
            Ok(Async::Ready(x)) => {
                //dbg!(&x);
                serde_json::from_value(x).map(Async::Ready).map_err(Into::into) },
...
pub fn build_request(id: usize, method: &str, mut params: Vec<rpc::Value>) -> rpc::Call {
    // by tur461: second issue resolved by replacing second parameter (block_hash) with string
    // "latest" on the request
    if method == "eth_call" {
        params.remove(1); // first remove the default second parameter
        params.push(jsonrpc_core::Value::from("latest"));
        //dbg!("id: {}, method: {}, params: {:?}", &id, &method, &params);
    }
...
pub fn to_result_from_output(output: rpc::Output) -> Result<rpc::Value, Error> {
    match output {
        rpc::Output::Success(success) => {
            let mut tmp = success.result.to_string();
            let re = Regex::new("\"(?i)xdc(?P<h>(?i)[0-9a-f]+)\"").unwrap();     
            tmp = re.replace_all(&tmp, "\"0x$h\"").to_string();
            Ok(tmp.parse()?)
        },
